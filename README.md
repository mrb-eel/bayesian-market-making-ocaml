...
The OCaml code is the primary implementation. The checked-in CSVs and figures were generated by the independent NumPy implementation because the assembly environment did not contain an OCaml toolchain. Every output directory carries an `engine` field in `run_manifest.csv`, and running the OCaml executable replaces it with `ocaml`. No result should be attributed to an engine other than the one named in that manifest.
