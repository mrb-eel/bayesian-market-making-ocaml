...
The numerical values in this document correspond to the \ResultsProfile\ run recorded in the manifest, using seed \ResultsSeed\ and engine \texttt{\ResultsEngine}. For the checked-in reference results, the authoritative provenance record is \texttt{results/data/run\_manifest.csv}, which identifies the OCaml implementation as the generating engine. These reference outputs were produced by the repository's manually triggered \texttt{full-reproduction} GitHub Actions workflow, which runs the complete OCaml workload from a clean Linux environment, validates the generated datasets, rebuilds the figures and report, records checksums, and uploads the resulting release directory as an artifact.

A separate Python implementation repeats the model equations rather than wrapping or calling the OCaml executable. Its purpose is to provide an independent numerical cross-check for sign, conditioning, posterior-timing, accounting, and aggregation disagreements. It is not the source of the checked-in reference results.
...
The main continuous-integration workflow builds the OCaml source, runs the test suite, executes the reduced experiment profile, validates the fresh CSV files, regenerates figures and LaTeX fragments, and compiles the report. The \texttt{full-reproduction} workflow performs the same pipeline at the full Monte Carlo workload and is the reproducible path used for the checked-in reference datasets, figures, and paper. The CSV outputs retain an explicit engine label so that numerical provenance is read from the manifest rather than inferred from the repository language.
